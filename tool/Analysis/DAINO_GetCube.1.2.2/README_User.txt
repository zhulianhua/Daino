...
DAINO_GetCube : construct the uniform-mesh data from the DAINO output file
...
**     NOTE     **
******************
1. This tool can be used to cut a slice or 3D rectangle from the DAINO output
   ...
   file of DAINO_GetCube will contain a uniform-mesh data with the spatial
   ...
   equal to that adopted in the parameter file of DAINO.
...
3. Usage demo:
   ...
   (1) Output the xy plane with x=[100:500], y=[1000:1234], and z=5 at level 2.
       ...
       mpirun -np 8 ./DAINO_GetCube -i Input -l 2 -n 3 -x 100 -X 400 
   ...
   (2) Output the x projection with y=[0:BoxSize] and z=[0:BoxSize] at level 5.
       ...
       mpirun -np 4 ./DAINO_GetCube -i Input -l 5 -n 4 -I 1 -r 4 -b
   ...
   (3) Output the 3D data with x=[x0:x0+dx], y=[y0:y0+dy], and z=[z0:z0+dz] 
       ...
       mpirun -np 1 ./DAINO_GetCube -i Input -o .txt -l 3 -n 7  
                                    -x x0 -X dx -y y0 -Y dy -z z0 -Z dz


4. A simple MATLAB script "DAINO_PlotSlice.m" is put in the directory
