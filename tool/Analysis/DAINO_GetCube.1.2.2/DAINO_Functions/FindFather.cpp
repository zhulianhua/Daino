...
void FindFather( const int lv )
{
   ...
   {
      Corner = patch.ptr[lv][PID0]->corner;
...
//    b. find the father patch 
      for (int FaLv=1; FaLv<lv; FaLv++)
      {
         GrandPaLv      = FaLv - 1;
         GrandPaPID     = FaPID;
         GrandPaCorner  = patch.ptr[GrandPaLv][GrandPaPID]->corner;
         ...
         FaPID = patch.ptr[GrandPaLv][GrandPaPID]->son + LocalID;

      } // for (int FaLv=1; FaLv<lv; FaLv++)


//    c. store the relation
      patch.ptr[lv-1][FaPID]->son = PID0;
      
      for (int PID=PID0; PID<PID0+8; PID++)        patch.ptr[lv][PID]->father = FaPID;
