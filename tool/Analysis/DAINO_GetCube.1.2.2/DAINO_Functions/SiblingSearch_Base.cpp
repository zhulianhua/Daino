...
// initialize all siblings as -1
   for (int PID=0; PID<patch.num[0]; PID++)
   for (int s=0; s<26; s++)         
      patch.ptr[0][PID]->sibling[s] = -1;  
...
void FindBaseSibling( const int PID )
{
   ...
   ip = (patch.ptr[0][PID]->corner[0] - MyRank_X[0]*NX0[0]*scale) / (PATCH_SIZE*scale) + 2;
   jp = (patch.ptr[0][PID]->corner[1] - MyRank_X[1]*NX0[1]*scale) / (PATCH_SIZE*scale) + 2;
   kp = (patch.ptr[0][PID]->corner[2] - MyRank_X[2]*NX0[2]*scale) / (PATCH_SIZE*scale) + 2;
...
// record the sibling relation
   for (int s=0; s<26; s++)      patch.ptr[0][PID]->sibling[s] = BaseP[ Sib[s] ];
