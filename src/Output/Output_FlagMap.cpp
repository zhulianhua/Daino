...
// set up parameters for outputing different slice
   ...
   char FileName[100];
   sprintf( FileName, "FlagMap_%d_%d_%2s", DAINO_RANK, lv, Dim[1] );
   if ( comment != NULL )       
   {
      strcat( FileName, "_" );
...
// record the flag result in the FlagMap
   ...
   for (int P=0; P<patch->num[lv]; P++)
   {
      ip = ( patch->ptr[0][lv][P]->corner[0]-MPI_Rank_X[0]*NX0[0]*scale0 ) / ( PATCH_SIZE*scale ) + 2;
      jp = ( patch->ptr[0][lv][P]->corner[1]-MPI_Rank_X[1]*NX0[1]*scale0 ) / ( PATCH_SIZE*scale ) + 2;
      kp = ( patch->ptr[0][lv][P]->corner[2]-MPI_Rank_X[2]*NX0[2]*scale0 ) / ( PATCH_SIZE*scale ) + 2;
...
//    the ID should never repeat
      ...
      if ( patch->ptr[0][lv][P]->flag )   FlagMap[ID] = 'O';
      else                                FlagMap[ID] = 'X';
   }
