...
// Description :  Verify five things:
//                1. The corner coordinates of all patches (real+buffer) in all ranks lie inside
//                   the simulation box (padded with two buffer patches on each side in each direction)
...
//                   (unless they lie outside the simulation box)
//
...
// check
   ...
   const int PScale             = PATCH_SIZE*patch->scale[lv];
   const int PGScale            = 2*PScale;
   const int BoxScale_Padded[3] = { patch->BoxScale[0] + 2*PGScale,
                                    patch->BoxScale[1] + 2*PGScale,
                                    patch->BoxScale[2] + 2*PGScale };
   int   NReal = patch->NPatchComma[lv][ 1];
   int   NTot  = patch->NPatchComma[lv][27];
   ...
   long *Cr1D_Real=NULL, *Cr1D_Buff=NULL;
   char *Match=NULL;
   long *Cr1D      = new long [ NReal+NBuff ];
   long *Cr1D_Sort = new long [ NReal+NBuff ];
   ...
   for (int TargetRank=0; TargetRank<MPI_NRank; TargetRank++)
   {
      if ( MPI_Rank == TargetRank )
      {
         for (int PID=0; PID<NTot; PID++)
         {
            for (int d=0; d<3; d++)    
            {
               Cr_Padded[d] = patch->ptr[0][lv][PID]->corner[d] + PGScale;

//             check 1 
               if ( PID < NReal ) // real patches
               {
                  if ( Cr_Padded[d] < PGScale  ||  Cr_Padded[d] > BoxScale_Padded[d]-PGScale-PScale )
                  {
                     ...
                     Aux_Message( stderr, "Check 1, Rank %4d, PID %8d (real patch): Cr_Padded[%d] = %7d\n",
...
//       check 3         
         memcpy( Cr1D_Sort, Cr1D, NTot*sizeof(long) );
         ...
         for (int t=0; t<NTot-1; t++)
         {
            if ( Cr1D_Sort[t] == Cr1D_Sort[t+1] )
            {
               ...
               Aux_Message( stderr, "Check 3, Rank %4d: Cr1D_Sort[%d] %8ld\n", MPI_Rank, t, Cr1D_Sort[t] );
            }
         }
      } // if ( MPI_Rank == TargetRank )
...
#  endif
   ...
   if ( MPI_Rank == 0 )
   {
      ...
      Cr1D_Real = new long [NReal_Tot];
      Cr1D_Buff = new long [NBuff_Tot];
      Match     = new char [NBuff_Tot];
   }
   
#  ifndef SERIAL
   MPI_Gatherv( Cr1D,       NReal, MPI_LONG, Cr1D_Real, NReal_Each, NReal_Disp, MPI_LONG, 0, MPI_COMM_WORLD );
   MPI_Gatherv( Cr1D+NReal, NBuff, MPI_LONG, Cr1D_Buff, NBuff_Each, NBuff_Disp, MPI_LONG, 0, MPI_COMM_WORLD );
#  else
...
//    apply periodic boundary condition for the external buffer patches     
      for (int t=0; t<NBuff_Tot; t++)
      {
         ...
         for (int d=0; d<3; d++)
         {
            if      ( Cr_Padded[d] < PGScale )                             Cr_Padded[d] += patch->BoxScale[d];
            else if ( Cr_Padded[d] > BoxScale_Padded[d]-PGScale-PScale )   Cr_Padded[d] -= patch->BoxScale[d];
         }
...
//    check 4      
      for (int t=0; t<NReal_Tot-1; t++)
      {
         if ( Cr1D_Real[t] == Cr1D_Real[t+1] )
         {
            ...
            Aux_Message( stderr, "Check 4, Rank %4d: Cr1D_Real[%d] %8ld\n", MPI_Rank, t, Cr1D_Real[t] );
         }
      }


//    check 5      
      Mis_Matching( NReal_Tot, Cr1D_Real, NBuff_Tot, Cr1D_Buff, Match );
      ...
      for (int t=0; t<NBuff_Tot; t++)
      {
         if ( Match[t] != 1 )
         {
            ...
            Aux_Message( stderr, "Check 5, Rank %4d: Cr1D_Buff[%d] %8ld\n", MPI_Rank, t, Cr1D_Buff[t] );
         }
      }
   } // if ( MPI_Rank == 0 )
