...
// this check function must work with the table "FlagTable_Rho"
            ...
            if ( patch->ptr[0][lv][PID]->son == -1 )
            {
               ...
               for (int i=0; i<PATCH_SIZE; i++)
               {
                  ...
                  if ( Rho > FlagTable_Rho[lv] )
                  {
                     ...
                     Aux_Message( stderr, "%4d\t%7d\t\t(%5d,%5d,%5d)\t\t(%2d,%2d,%2d)\t%14.7e\n",
                                  MPI_Rank, PID, patch->ptr[0][lv][PID]->corner[0],  
                                                 patch->ptr[0][lv][PID]->corner[1],  
                                                 patch->ptr[0][lv][PID]->corner[2], i, j, k, Rho ); 
   ...
   else
   {
      ...
      MPI_Reduce( &patch->NPatchComma[lv][1], &NPatch_Sum , 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD );
