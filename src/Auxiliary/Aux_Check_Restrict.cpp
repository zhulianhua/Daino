...
#  endif
   ...
   const int FSg = patch->FluSg[lv+1];
   const int CSg = patch->FluSg[lv  ];

#  ifdef FLOAT8
   const real TolErr = 1.e-13;
#  else
   const real TolErr = 1.e-5;
#  endif
   int Pass = true;

   int SonPID0, SonPID, ii0, jj0, kk0, ii, jj, kk;
   real ResData[NCOMP][PATCH_SIZE][PATCH_SIZE][PATCH_SIZE];
   ...
   for (int TargetRank=0; TargetRank<MPI_NRank; TargetRank++)
   {
      if ( MPI_Rank == TargetRank )
      {
         ...
         {
            SonPID0 = patch->ptr[0][lv][PID]->son;
...
//             fill up the restrict-data array
               for (int LocalID=0; LocalID<8; LocalID++)
               {
                  ...
                  for (int i=0; i<PATCH_SIZE; i++)    {  ii = ii0 + i/2; 
                     ...
                     ResData[v][kk][jj][ii] += 0.125*patch->ptr[FSg][lv+1][SonPID]->fluid[v][k][j][i];
...
//             compare the data of the restrict-data array and the data stored in the patch pointers
               ...
               for (int i=0; i<PATCH_SIZE; i++)  
               {
                  u = patch->ptr[CSg][lv][PID]->fluid[v][k][j][i];
                  ...
                  if ( Err > TolErr ) 
                  {
                     ...
                     Aux_Message( stderr,"%4d\t%7d\t(%5d,%5d,%5d)\t(%2d,%2d,%2d)\t%7d\t%14.7e\t%14.7e\t%14.7e\n",
