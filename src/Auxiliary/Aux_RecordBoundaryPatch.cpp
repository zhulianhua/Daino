...
void Aux_RecordBoundaryPatch( const int lv, int *NList, int **IDList, int **PosList )
{
   ...
   const int scale  = patch->scale[lv];      // Pos      : Patch position in (x,y,z) direction
...
// scan over all patches at level "lv" to record the IDS of boundary patches
   ...
   {
      LCorner[0]  = patch->ptr[0][lv][P]->corner[0] - MPI_Rank_X[0]*NX0[0]*scale0;
      LCorner[1]  = patch->ptr[0][lv][P]->corner[1] - MPI_Rank_X[1]*NX0[1]*scale0;
      LCorner[2]  = patch->ptr[0][lv][P]->corner[2] - MPI_Rank_X[2]*NX0[2]*scale0;
