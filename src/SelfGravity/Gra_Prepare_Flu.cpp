...
#  pragma omp parallel for private( N, PID, PID0 )
...
#        if ( MODEL == ELBDM )
         ...
         for (int i=0; i<PATCH_SIZE; i++)
            h_Flu_Array_G[N][v][k][j][i] = patch->ptr[ patch->FluSg[lv] ][lv][PID]->fluid[v+1][k][j][i];
...
#        else
         ...
         for (int j=0; j<PATCH_SIZE; j++)
         for (int i=0; i<PATCH_SIZE; i++)
            h_Flu_Array_G[N][v][k][j][i] = patch->ptr[ patch->FluSg[lv] ][lv][PID]->fluid[v][k][j][i];
#        endif 
      }
   }
