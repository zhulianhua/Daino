...
// DumpByTime : true --> dump data according to the physical time
   const bool DumpByTime = (    ( OPT__OUTPUT_TOTAL || OPT__OUTPUT_PART || OPT__OUTPUT_ERROR || OPT__OUTPUT_BASEPS )
                             && ( OPT__OUTPUT_MODE == OUTPUT_CONST_DT || OPT__OUTPUT_MODE == OUTPUT_USE_TABLE )  )
                           ? true : false;
...
#     elif ( MODEL == ELBDM )
      fprintf( File, "Gravity   : dt = %12.6e, dTime = %12.6e, lv = %2d, MaxPot = %13.6e\n", 
               dt2, dTime2, MinDtLv_Gravity, MinDtVar_Gravity );
#     else
#     error : ERROR : unsupported MODEL !!
#     endif // MODEL
