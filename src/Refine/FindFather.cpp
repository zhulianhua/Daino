...
void FindFather( const int lv, const int Mode )
{
   ...
   const int NBaseP[3]   = {  NX0[0]/PATCH_SIZE + 4, 
                              NX0[1]/PATCH_SIZE + 4, 
                              NX0[2]/PATCH_SIZE + 4  };
   const int RankDisp[3] = {  DAINO_RANK_X(0)*NX0[0]*scale0, 
                              DAINO_RANK_X(1)*NX0[1]*scale0, 
                              DAINO_RANK_X(2)*NX0[2]*scale0  };
   ...
   {
      Corner = patch->ptr[0][lv][PID0]->corner;
...
//       b. find the father patch 
         for (int FaLv=1; FaLv<lv; FaLv++)
         {
            GrandPaLv      = FaLv - 1;
            GrandPaPID     = FaPID;
            GrandPaCorner  = patch->ptr[0][GrandPaLv][GrandPaPID]->corner;
            FaPS           = PATCH_SIZE*patch->scale[FaLv];
            ...
            FaPID = patch->ptr[0][GrandPaLv][GrandPaPID]->son + LocalID;
      ...
      {
         for (int FaPID_Candidate=0; FaPID_Candidate<patch->NPatchComma[lv-1][1]; FaPID_Candidate++)
         {
            FaCorner = patch->ptr[0][lv-1][FaPID_Candidate]->corner;
...
#           ifdef DAINO_DEBUG
            if ( FaPID_Candidate == patch->NPatchComma[lv-1][1]-1 )
...
//    store the relation
      patch->ptr[0][lv-1][FaPID]->son = PID0;
      
      for (int PID=PID0; PID<PID0+8; PID++)  patch->ptr[0][lv][PID]->father = FaPID;
