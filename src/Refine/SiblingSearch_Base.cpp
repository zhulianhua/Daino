...
#  ifndef SERIAL
   for (int s=0; s<26; s++)
   for (int PID0=patch->NPatchComma[0][s+1]; PID0<patch->NPatchComma[0][s+2]; PID0+=8)
...
//
// Parameter   :  PID : Index of the targeted patch
//-------------------------------------------------------------------------------------------------------
void FindBaseSibling( const int PID )
{
   ...
   for (int d=0; d<3; d++)
      order[d] = (patch->ptr[0][0][PID]->corner[d] - DAINO_RANK_X(d)*NX0[d]*scale0) / (PATCH_SIZE*scale0) + 2;
...
// record the sibling relation
   for (int s=0; s<26; s++)      patch->ptr[0][0][PID]->sibling[s] = BaseP[ Sib[s] ];
