...
#  ifdef ENFORCE_POSITIVE
   L.Egy = FMAX( L.Egy, MIN_VALUE );
   R.Egy = FMAX( R.Egy, MIN_VALUE );
#  endif
...
// right complete solution
   ...
   else                     // right rarefaction
   {
      ...
      real a_star = SQRT( Gamma*R_star.Egy/R_star.Egy ); // sound speed of right star region
...
// solve speed of waves
   ...
   if ( L.Egy < L_star.Egy )  
      eival.Rho = L.Px - SQRT( Gamma*L.Egy/L.Rho )
                        *SQRT(  (real)0.5*_Gamma*( Gamma_p1*L_star.Egy/L.Egy + Gamma_m1 )  ); // left shock
   else                      
      eival.Rho = L.Px - SQRT( Gamma*L.Egy/L.Rho ); // left rarefaction

   if ( R.Egy < R_star.Egy )  
      eival.Egy = R.Px + SQRT( Gamma*R.Egy/R.Rho )
                        *SQRT(  (real)0.5*_Gamma*( Gamma_p1*R_star.Egy/R.Egy + Gamma_m1 )  ); // right shock
   else                      
