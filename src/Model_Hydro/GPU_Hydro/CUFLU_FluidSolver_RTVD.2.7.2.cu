...
//    if ( i > 0 )
      ...
      {
         for (int v=0; v<5; v++)    
            Fluid_half[v] = Fluid[v] - _dh*dt_half*( s_flux[ty][v][i] - s_flux[ty][v][im] ) ;


//       enforce the pressure to be positive
#        ifdef ENFORCE_POSITIVE
         Ek            = (real)0.5*( Fluid_half[1]*Fluid_half[1] + Fluid_half[2]*Fluid_half[2] + 
                                     Fluid_half[3]*Fluid_half[3] ) / Fluid_half[0];
         TempPres      = Gamma_m1*( Fluid_half[4] - Ek );
         TempPres      = FMAX( TempPres, MIN_VALUE );
         Fluid_half[4] = Ek + _Gamma_m1*TempPres;
#        endif
      }
...
//    (b2). set the right-moving flux defined in the right-hand surface by the TVD scheme
      ...
      {
         for (int v=0; v<5; v++)
            s_RLflux[ty][v][i] = (real)0.5*( s_cu[ty][v][i] + s_cw[ty][v][i] );
      }
...
//    if ( i < FLU_NXT-2 )
      ...
      {
         for (int v=0; v<5; v++)
            s_RLflux[ty][v][i] = (real)0.5*( s_cu[ty][v][ip] - s_cw[ty][v][ip] );
      }
...
//       enforce the pressure to be positive
#        ifdef ENFORCE_POSITIVE
         Ek       = (real)0.5*( Fluid[1]*Fluid[1] + Fluid[2]*Fluid[2] + Fluid[3]*Fluid[3] ) / Fluid[0];
         TempPres = Gamma_m1*( Fluid[4] - Ek );
         TempPres = FMAX( TempPres, MIN_VALUE );
         Fluid[4] = Ek + _Gamma_m1*TempPres;
#        endif
