...
#  ifdef ENFORCE_POSITIVE
   L[4] = FMAX( L[4], MIN_VALUE );
   R[4] = FMAX( R[4], MIN_VALUE );
#  endif
...
// solve speed of waves
   ...
   if ( L[4] < L_star[4] ) // left shock 
      eival[0] = L[1] - SQRT( Gamma*L[4]/L[0] )*SQRT(  (real)0.5/Gamma*( Gamma_p1*L_star[4]/L[4] + Gamma_m1 )  );
   else                    // left rarefaction                   
      eival[0] = L[1] - SQRT ( Gamma*L[4]/L[0] );

   if ( R[4] < R_star[4] ) // right shock
      eival[4] = R[1] + SQRT( Gamma*R[4]/R[0] )*SQRT(  (real)0.5/Gamma*( Gamma_p1*R_star[4]/R[4] + Gamma_m1 )  );
   else                    // right rarefaction                  
